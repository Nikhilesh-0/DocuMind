...
Purpose:
    ...
    UPDATED (Feb 2026): Switched to LOCAL execution using "all-MiniLM-L6-v2".
    This runs inside the Streamlit container, removing the need for external 
    API keys and fixing "InferenceClient" connection errors.
...
def get_vector_store(text_chunks, session_id):
    """
    Creates FAISS vector store from text chunks using LOCAL HuggingFace embeddings.
    ...
    logger.info(f"Starting local vector store creation for session {session_id[:8]}... "
                f"with {len(text_chunks)} chunks")

    try:
        # 2. Initialize Local Model (Downloads automatically on first run)
        # using 'all-MiniLM-L6-v2': lightweight, fast, and good for RAG
        logger.info("Loading local embedding model (cpu)...")
        embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")

        # 3. Create Vector Store
        # Processing is done locally. No batching logic needed for <5000 chunks.
        logger.info("Generating embeddings locally...")
        vector_store = FAISS.from_texts(texts=text_chunks, embedding=embeddings)

        # 4. Save to Session-Specific Directory
...
def load_vector_store(session_id):
    ...
    try:
        # 2. Re-initialize the same Local Model
        embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")

        # 3. Load FAISS Index
        # allow_dangerous_deserialization is required for FAISS
...
if __name__ == "__main__":
    print("Local Embeddings Module - Ready")
# """
# Embeddings & Vector Store Module - Knowledge Base Creation Layer
# ==================================================================
